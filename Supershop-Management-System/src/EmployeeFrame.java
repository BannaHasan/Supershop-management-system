...
class EmployeeFrame extends JFrame implements ActionListener {
	...
	public static Connection ConnectDB() {
		try {
			Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/smsdb", "root","root");
			return conn;
		} catch (Exception e1) {
	...
	public EmployeeFrame() {
		...
		cartTable.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Product ID", "Product Name", "UnitPrice", "Quantity", "Amount" }));
		...
		tShowAllProduct.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Product ID", "Product Name", "UnitPrice", "Stock" }));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (cartQuantity > updateQuantity) {
						...
						String sql = "UPDATE productdetails SET quantity='" + finalProductStock + "' WHERE productID='"
								+ product.getId() + "' ";
		...
		bRemove.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String sql = "UPDATE productdetails SET quantity='" + productStock + "' WHERE productID='"
							+ product.getId() + "' ";
					st = (Statement) conn.prepareStatement(sql);
					st.execute(sql);
				} catch (Exception ac) {
	...
	public ArrayList<Product> getProductlist() {
		...
		try {
			...
			while (rs.next()) {
				p = new Product(rs.getString("productID"), rs.getString("productName"), rs.getDouble("unitPrice"),
	...
	public ArrayList<Product> getProductlist2(String query) {
		...
		try {
			...
			rs = st.executeQuery(query);
			while (rs.next()) {
				product = new Product(rs.getString("productID"), rs.getString("productName"), rs.getDouble("unitPrice"),
						rs.getDouble("quantity"));
